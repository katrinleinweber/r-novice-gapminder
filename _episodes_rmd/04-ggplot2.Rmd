...
built from the same set of components: a **data set**, an
**aesthetic mapping**, and **geometric layers**:

* **Data sets** are the data that you, the user, provide.

* **Aesthetic mappings** are what connect the data to the graphics.
...
* **Geometric layers** are the actual graphical output from ggplot2. Layers
...
Now we're going to add in the **aesthetic mapping** using the
...
visually represent the data. We do this by adding a new **geometric layer**
to the plot using one of the **geom_...** functions.
...
> ## Challenge 1
>
> Modify the above example so that the figure shows how life expectancy has
> changed over time.
>
> Hint: use `names(gapminder)` or `str(gapminder)` to find a time-related variable to plot
> on the x-axis.
>
> > ## Solution
> >
...
ggplot(americas, aes(year, lifeExp)) +
  geom_line() +
  facet_wrap(vars(country)) +
  theme(axis.text.x = element_text(angle = 45))
```

The `facet_wrap` layer took a `vars()` function as its argument. It tells R to draw a panel for each unique value in the `country` variable
of the gapminder dataset. You will also encounter [older code](https://www.tidyverse.org/blog/2018/07/ggplot2-3-0-0/#tidy-evaluation)
that still uses a "formula"-based syntax with `~`.
...
```{r save}
lifeExp_plot <- ggplot(americas, aes(year, lifeExp, color = continent)) +
  geom_line() +
  facet_wrap(vars(country)) +
  labs(
    x = "Year",              # x axis title
